<?php
// exit(); // à enlever si besoind e relancer la BDD
        ...
        if($row == 1) {
            ...
            if(trim($data[4]) != '') {
                ...
                foreach($lesIngredients as $ingredient) {
                    ...
                    if($ingredient != "") {
                        if(!isset($ingredients[$ingredient])  && ! array_key_exists($ingredient, $ingredients)) {
                            // $addIngredients = INSERT INTO `ingredient` (`name`, `picture`, `alt`, `active`, `created_at`) VALUES (?, ?, ?, '1', NOW())
            ...
            if(trim($data[5]) != '') {
                ...
                foreach($needs as $need) {
                    ...
                    if($need != "") {
                        if(!isset($besoins[$need]) && !array_key_exists($need, $besoins)) {
                            // $addBesoins = INSERT INTO `skinneed` (`name`, `active`, `created_at`) VALUES (?, '1', NOW())
